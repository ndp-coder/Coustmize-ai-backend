...
if (!GEMINI_API_KEY || !JWT_SECRET || !DATABASE_URL) {
    console.error("FATAL ERROR: Make sure GEMINI_API_KEY, JWT_SECRET, and DATABASE_URL are defined in your .env file.");
    process.exit(1);
}
...
app.post('/api/profile', authenticateToken, async (req, res) => {
    const { name, age, classValue, classGroup, gender, stream } = req.body;
    await pool.query(
        `UPDATE profiles 
         SET name = $1, age = $2, class_value = $3, class_group = $4, gender = $5, stream = $6 
         WHERE user_id = $7`,
        [name, age, classValue, classGroup, gender, stream, req.user.id]
    );
...
app.get('/api/chats', authenticateToken, async (req, res) => {
    const result = await pool.query("SELECT id, title FROM chats WHERE user_id = $1 ORDER BY created_at DESC", [req.user.id]);
...
app.post('/api/chats/new', authenticateToken, async (req, res) => {
    ...
    const newChatResult = await pool.query(
        "INSERT INTO chats (user_id, title, history) VALUES ($1, $2, $3) RETURNING id, title, history",
        [req.user.id, 'New Chat', JSON.stringify([
...
app.get('/api/chats/:id', authenticateToken, async (req, res) => {
    const result = await pool.query("SELECT * FROM chats WHERE id = $1 AND user_id = $2", [req.params.id, req.user.id]);
...
app.post('/api/chats/save', authenticateToken, async (req, res) => {
    ...
    await pool.query(
        "UPDATE chats SET title = $1, history = $2 WHERE id = $3 AND user_id = $4",
        [chat.title, JSON.stringify(chat.history), chat.id, req.user.id]
    );
...
app.put('/api/chats/:id/rename', authenticateToken, async (req, res) => {
    ...
    await pool.query("UPDATE chats SET title = $1 WHERE id = $2 AND user_id = $3", [title, req.params.id, req.user.id]);
    res.status(200).json({ message: "Chat renamed." });
});
...
app.post('/api/gemini', authenticateToken, async (req, res) => {
    ...
    if (tool) {
        const toolSystemPrompt = buildSystemPrompt(profile); // Get the full persona prompt
        if (tool === "planner") {
            prompt = `Using your persona defined by the following instructions, create a study plan. Instructions: "${toolSystemPrompt}". Now, create a detailed 7-day study plan for the topic "${topic}".`;
        } else if (tool === "quiz") {
            prompt = `Using your persona defined by the following instructions, create a quiz. Instructions: "${toolSystemPrompt}". Now, generate a 5-question multiple-choice quiz on the topic "${topic}".`;
            generationConfig = {
